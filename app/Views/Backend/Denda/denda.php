...
if (!empty($data_denda)) {
    foreach ($data_denda as $denda) {
        ...
        $sisa_denda = $total_denda - $denda_dibayar; // Perhitungan Sisa Denda
?>
    <tr>
        <td><?= $no++; ?></td>
...
<td>Rp. <?= number_format($sisa_denda, 0, ',', '.'); ?></td> <!-- Tampilkan Sisa Denda -->
        <td>
            <a href="<?= base_url('admin/detail-denda/' . $denda['id_peminjaman']); ?>" class="btn btn-sm btn-info">Detail</a>
        </td>
    </tr>
<?php
    }
} else {
    echo '<tr><td colspan="8" class="text-center">Tidak ada data denda.</td></tr>';
}
?>
</tbody>
        </table>
